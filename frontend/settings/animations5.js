...
async function fetchAccountInfo() {
  try {
    ...
    if (response.ok) {
      ...
      if (userNameElement)
        userNameElement.textContent = accountData.name || "Unknown";
      if (userEmailElement)
        userEmailElement.textContent = accountData.email || "Unknown email";
      if (userAvatarElement)
        userAvatarElement.src = accountData.avatar || userAvatarElement.src;
    } else {
...
async function checkBudget() {
  const weeklyLimit = parseFloat(
    document.getElementById("weekly-slider").value
  );
  const monthlyLimit = parseFloat(
    document.getElementById("monthly-slider").value
  );
  const annualLimit = parseFloat(
    document.getElementById("annually-slider").value
  );
  ...
  if (weeklyExpenses > weeklyLimit) {
    alert(
      `Warning: Your weekly expenses ($${weeklyExpenses.toFixed(
        2
      )}) exceed your weekly limit of $${weeklyLimit.toFixed(2)}!`
    );
  }

  if (monthlyExpenses > monthlyLimit) {
    alert(
      `Warning: Your monthly expenses ($${monthlyExpenses.toFixed(
        2
      )}) exceed your monthly limit of $${monthlyLimit.toFixed(2)}!`
    );
  }

  if (annualExpenses > annualLimit) {
    alert(
      `Warning: Your annual expenses ($${annualExpenses.toFixed(
        2
      )}) exceed your annual limit of $${annualLimit.toFixed(2)}!`
    );
  }
}
...
async function updateBudgetUI() {
  ...
  document.getElementById("budget-limit").textContent = `$${budgetLimit.toFixed(
    2
  )}`;
